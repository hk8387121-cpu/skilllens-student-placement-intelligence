...
async function startServer() {
  ...
  app.get('/api/health', (req, res) => {
    res.json({ status: 'ok', service: 'Student Placement Intelligence System', records: db.getAllRecords().length });
  });
  ...
  app.post('/api/ml/predict', (req, res) => {
    ...
    if (!Array.isArray(features) || features.length !== 10 || features.some((v: any) => typeof v !== 'number' || !Number.isFinite(v))) {
      return res.status(400).json({ error: 'features must be an array of 10 numeric values' });
    }
